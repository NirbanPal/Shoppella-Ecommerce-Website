...
public class GlobalExceptionHandler {
	...
	public ResponseEntity<MyErrorClass> handleMaxSizeException(MaxUploadSizeExceededException exc) {
		// You can create a custom error response object as well
		MyErrorClass e1 = new MyErrorClass();
		e1.setMessage("File is too large! The maximum allowed size is 1MB.");
		e1.setLocalDateTimes(LocalDateTime.now());
		e1.setDesc(exc.getMessage());

		return new ResponseEntity<MyErrorClass>(e1, HttpStatus.PAYLOAD_TOO_LARGE);
	}
	...
	public ResponseEntity<MyErrorClass> getException(MethodArgumentNotValidException e, WebRequest req) {
		...
		MyErrorClass e1 = new MyErrorClass();
		e1.setMessage(Objects.requireNonNull(e.getBindingResult().getFieldError()).getDefaultMessage());
		e1.setLocalDateTimes(LocalDateTime.now());
	...
	public ResponseEntity<MyErrorClass> getException(InvalidFileException e, WebRequest req) {
